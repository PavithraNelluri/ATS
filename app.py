...
class Resume_Analyser:
    ...
    def skills_information(self,resume_skills,job_skills):
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are an expert at finding redundant data and removing them.Your job is to give the list of cleaned matched skills and missing skills by comparing the 
                 examining the missing_skills and matched_skills which I provide.
                 If you find any skill that is in both matched_skills and missing_skills remove that from
                 missing_skills.
                 you should output only the Matched_skills and Missing_skills as output in JSON format and don't provide any other explanation.
                 **Remember provide only JSON as output """),
                ("user", "missing_skills:\n{missing_skills}\n\nmatched_skills:\n{matched_skills}")
            ])
    ...
    def course_recommender(self,missing_skills):
            ...
            prompt = ChatPromptTemplate.from_messages([
                ...
                "For each skill, return the top 3 high-quality famous websites for learning that skill as clickable URLs. "
                "Provide only valid links. Don't give invalid or undefined links."
    ...
    def get_resume_score(self,resume_data,job_data):
            llm = ChatGroq(model='gemma2-9b-it', groq_api_key=self.groq_api_key)
            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are an expert ATS.Your job is to give the resume Score by analysing the 
                 resume_data(extracted from resume) and job_data (extracted from job decsription)
                 you should output only the resume score in percentage and don't provide any other explanation.
                 **Remember provide only resume score in percentage as output."""),
                ("user", "resume_data:\n{resume_data}\n\njob_data:\n{job_data}")
            ])
            output_parser = StrOutputParser()
            chain = prompt | llm | output_parser
...
def job_seeker():
    ...
    if request.method == 'POST':
        try:
            ...
            resume_score =analyser.get_resume_score(resume_data,job_data)
            ...
            # Skill analysis
            skills_inf= analyser.skills_information(resume_skills, job_skills)
            print(skills_inf,type(skills_inf))
            missing_skills, matched_skills=skills_inf.get('Missing_skills',[]), skills_inf.get('Matched_skills',[])
